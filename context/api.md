...
⚠️ Контракт для «профиль пуст → нужна анкета» не задокументирован бэком
(вопрос 10). Временно пустой профиль = пустой `full_name`, функция
`isProfileComplete` в `me.service.ts`.
...
| PATCH  | `/v1/me/profile/`                       | `me.service` | Анкета: `completeProfile` шлёт `full_name`, `phone` (пустой телефон не шлётся). «Откуда узнали» – `MOCK(profile-referral)` |
...
| GET    | `/v1/me/bookings/` _(рабочее название)_ | `me.service` | **Ещё не существует на бэке.** `getBookings` → `MeBooking[]` на `MOCK(bookings)`, см. `architecture.md` §6                 |
